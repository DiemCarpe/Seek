...
import com.seek.view.ViewfinderResultPointCallback;
...
//        } else if (message.what == com.google.zxing.client.android.R.id.decode_failed) {
...
//        }
        ...
        switch (message.what) {
            case com.seek.R.id.restart_preview:
                restartPreviewAndDecode();
                break;
            case com.seek.R.id.decode_succeeded:
                state = State.SUCCESS;
                Bundle bundle = message.getData();
                Bitmap barcode = null;
                float scaleFactor = 1.0f;
                ...
                activity.handleDecode((Result) message.obj, barcode, scaleFactor);
                break;
            case com.seek.R.id.decode_failed:
                // We're decoding as fast as possible, so when one decode fails, start another.
                state = State.PREVIEW;
                cameraManager.requestPreviewFrame(decodeThread.getHandler(), com.seek.R.id.decode);
                break;
            case com.seek.R.id.return_scan_result:
                activity.setResult(Activity.RESULT_OK, (Intent) message.obj);
                activity.finish();
                break;
//            case com.seek.R.id.launch_product_query:
//                String url = (String) message.obj;
//
//                Intent intent = new Intent(Intent.ACTION_VIEW);
//                intent.addFlags(Intents.FLAG_NEW_DOC);
...
//                break;
    ...
    public void quitSynchronously() {
        ...
        Message quit = Message.obtain(decodeThread.getHandler(), com.seek.R.id.quit);
        quit.sendToTarget();
        try {
        ...
        removeMessages(com.seek.R.id.decode_succeeded);
        removeMessages(com.seek.R.id.decode_failed);
    }
